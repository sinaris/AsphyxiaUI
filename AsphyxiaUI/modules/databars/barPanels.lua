...
for i = 1, S.maxDatabars do
	...
	S.databars[i].text:SetPoint("TOPRIGHT", S.databars[i].statusbar, "TOPRIGHT", -2, -2)
	S.databars[i].text:SetPoint("BOTTOMLEFT", S.databars[i].statusbar, "BOTTOMLEFT", 2, 2)
end
...
local function hideDatabars(self)
	...
	self.text:SetText(S.datacolor.."Open")
	self:ClearAllPoints()
...
local function showDatabars(self)
	...
	self.text:SetText(S.datacolor.."Close")
	self:ClearAllPoints()
...
S.databars["toggle"].text:SetText(S.datacolor.."Close")
