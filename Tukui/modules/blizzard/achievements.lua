...
AchievementHolder.text = S.SetFontString( AchievementHolder, C["media"].pixelfont, C["datatext"].fontsize, "MONOCHROMEOUTLINE" )
AchievementHolder.text:SetPoint( "CENTER" )
AchievementHolder.text:SetText( L.move_achievements )

local pos = "TOP"

function S.AchievementMove( self, event, ... )
	local previousFrame
...
hooksecurefunc( "AchievementAlertFrame_FixAnchors", S.AchievementMove )
...
function S.PostAchievementMove( frame )
...
frame:SetScript( "OnEvent", function( self, event, ... ) S.AchievementMove( self, event, ... ) end )