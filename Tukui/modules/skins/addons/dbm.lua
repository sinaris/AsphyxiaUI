...
if not IsAddOnLoaded( "DBM-Core" ) or not C["Addon_Skins"].DBM == true then return end
	...
	for bar in self:GetBarIterator() do
		if not bar.injected then
				...
				if not timer.styled then	
					...
					timer:SetFont(C["media"].pixelfont, 12, "MONOCHROMEOUTLINE")
	...
	if not anchor.styled then
		local header={anchor:GetRegions()}
			if header[1]:IsObjectType("FontString") then
				header[1]:SetFont(C["media"].pixelfont, 12, "MONOCHROMEOUTLINE")
