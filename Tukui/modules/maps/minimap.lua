...
TukuiMinimap:Point( "TOPRIGHT", UIParent, "TOPRIGHT", -34, -10 )
TukuiMinimap:Size( 153 )
TukuiMinimap:SetClampedToScreen( true )
TukuiMinimap:SetMovable(true)
TukuiMinimap.text = S.SetFontString( TukuiMinimap, C["media"].pixelfont, C["datatext"].fontsize, "MONOCHROMEOUTLINE" )
...
local status
if( S.toc >= 40300 ) then status = LFGSearchStatus else status = LFDSearchStatus end
status:SetTemplate( "Transparent" )
...
Minimap:SetScript( "OnMouseUp", function( self, btn )
	...
	if( btn == "RightButton" ) then	
		if( position:match( "RIGHT" ) ) then xoff = S.Scale( -8 ) end
		ToggleDropDownMenu( 1, nil, MiniMapTrackingDropDown, TukuiMinimap, xoff, S.Scale( -2 ) )
	elseif( btn == "MiddleButton" ) then
		if not TukuiMicroButtonsDropDown then return end
		if( position:match( "RIGHT" ) ) then xoff = S.Scale( -160 ) end
		EasyMenu( S.MicroMenu, TukuiMicroButtonsDropDown, "cursor", xoff, 0, "MENU", 2 )
	else
