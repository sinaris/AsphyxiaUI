...
if( C["asphyxia"].layout ~= "asphyxia3" ) then return end
...
local font = C["media"].asphyxiafont
...
local function Shared( self, unit )
	...
	if( unit == "player" ) then
		...
		power:Size( 180, 45 )
		power:Point( "TOPRIGHT", health, "TOPRIGHT", 5, 0 )
		...
		if( C["unitframes"].charportrait == true ) then
			local portraitBorder = CreateFrame( "Frame", self:GetName() .. "_PortraitBorder", self )
			portraitBorder:CreatePanel( "Default", 67, 18, "TOPLEFT", health, "BOTTOMLEFT", -2, -4 )
			portraitBorder:CreateShadow( "Default" )

			local portrait = CreateFrame( "PlayerModel", nil, portraitBorder )
			portrait:SetFrameLevel( portraitBorder:GetFrameLevel() )
		...
		if( C["unitframes"].classicon == true ) then
			...
			classicon:CreatePanel( "Default", 29, 29, "TOPRIGHT", health, "TOPLEFT", -4, 2 )
		...
		if( C["unitframes"].classbar == true ) then
			...
			if( S.myclass == "DRUID" ) then
				...
				DruidManaBackground:Size( 200, 5 )
			...
			if( S.myclass == "WARLOCK" or S.myclass == "PALADIN" ) then
				...
				bars:Size( 200, 5 )
				bars:Point( "LEFT", health, "TOPLEFT", 12, 10 )
			...
			if( S.myclass == "DEATHKNIGHT" ) then
				...
				Runes:Point( "LEFT", health, "TOPLEFT", 12, 10 )
				Runes:Size( 200, 5 )
				...
				for i = 1, 6 do
					...
					if( i == 1 ) then
						...
						Runes[i]:SetWidth( S.Scale( 200 / 6 ) )
					else
			...
			if( S.myclass == "SHAMAN" ) then
				...
				for i = 1, 4 do
					...
					TotemBar[i]:SetWidth( S.Scale( 194 ) / 4 )
	...
	if( unit == "target" ) then
		...
		local power = CreateFrame( "StatusBar", nil, self )
		power:Size( 180, 45 )
		power:Point( "TOPLEFT", health, "TOPLEFT", -5, 0 )
		power:SetStatusBarTexture( normTex )
		power:SetFrameLevel( self.Health:GetFrameLevel() + 2 )
		power:SetFrameStrata( "BACKGROUND" )
		...
		if( C["unitframes"].charportrait == true ) then
			local portraitBorder = CreateFrame( "Frame", self:GetName() .. "_PortraitBorder", self )
			portraitBorder:CreatePanel( "Default", 67, 18, "TOPRIGHT", health, "BOTTOMRIGHT", 2, -4 )
			portraitBorder:CreateShadow( "Default" )
		...
		if( C["unitframes"].classicon == true ) then
			local classicon = CreateFrame( "Frame", self:GetName() .. "_ClassIconBorder", self )
			classicon:CreatePanel( "Default", 29, 29, "TOPLEFT", health, "TOPRIGHT", 4, 2 )
			classicon:CreateShadow( "Default" )
		...
		if( C["unitframes"].targetauras == true ) then
			...
			if( S.lowversion ) then
				...
				buffs:SetPoint( "BOTTOMLEFT", self, "TOPLEFT", -1, 1 )
			...
			if( S.lowversion ) then
				...
				debuffs:SetPoint( "BOTTOMLEFT", buffs, "TOPLEFT", -2, 5 )
				debuffs.size = 29
				debuffs.num = 16
			end
...
player:Size( 244, player.Health:GetHeight() + player.Power:GetHeight() + player.panel:GetHeight() + 6 )
target:Size( 244, target.Health:GetHeight() + target.Power:GetHeight() + target.panel:GetHeight() + 6 )
...
f:SetScript( "OnEvent", function( self, event, addon )
	...
	if( IsAddOnLoaded( "Tukui_Raid" ) ) then
		player:Point( "TOP", UIParent, "BOTTOM", -230 , 260 )
		target:Point( "TOP", UIParent, "BOTTOM", 230, 260 )
		tot:Point( "TOP", UIParent, "BOTTOM", 0 , 260 )
		pet:Point( "TOP", UIParent, "BOTTOM", 0 , 230 )
