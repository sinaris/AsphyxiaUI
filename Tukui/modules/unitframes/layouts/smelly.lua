...
if( C["asphyxia"].layout ~= "smelly" ) then return end
...
local function Shared( self, unit )
	...
	if( unit == "player" ) then
		...
		health.value:Point( "RIGHT", health, "RIGHT", -6, 3 )
		...
		if( C["unitframes"].classbar == true ) then
			...
			if( S.myclass == "DEATHKNIGHT" ) then
				...
				for i = 1, 6 do
					...
					if( i == 1 ) then
						...
						Runes[i]:Point( "TOPLEFT", Runes[i - 1], "TOPRIGHT", S.Scale( 8 ), 0 )
					end

					Runes[i].border = CreateFrame( "Frame", nil, Runes )
					...
					Runes[i].border:SetFrameLevel( 6 )
					Runes[i]:SetFrameLevel( 2 )
			...
			if( S.myclass == "SHAMAN" ) then
				...
				for i = 1, 4 do
					...
					TotemBar[i]:Size( S.Scale( 10 ), 7 )
					TotemBar[i]:SetMinMaxValues( 0, 1 )

					if( i == 1 ) then
						TotemBar[i]:Point( "TOPLEFT", health, "TOPLEFT", 7, 5 )
					else
						TotemBar[i]:Point( "TOPLEFT", TotemBar[i - 1], "TOPRIGHT", S.Scale( 8 ), 0 )
					end
		...
		if( C["unitframes"].combatfeedback == true ) then
			...
			CombatFeedbackText:SetPoint( "CENTER", 0, 3 )
	...
	if( unit == "target" ) then
		...
		Name:Point( "CENTER", health, "CENTER", 0, 3 )
	...
	if( unit == "pet" ) then
		...
		local Name = health:CreateFontString( nil, "OVERLAY" )
		Name:SetPoint( "CENTER", self.Health, "CENTER", 0, 3 )
		Name:SetFont( S.SetUserFont() )
		Name:SetJustifyH( "CENTER" )
		Name:SetShadowOffset( 1.25, -1.25 )
		self:Tag( Name, "[Tukui:getnamecolor][Tukui:namemedium]" )
	...
	if( unit and unit:find( "arena%d" ) and C["arena"].unitframes == true ) then
		...
		debuffs:SetWidth( 200 )
		debuffs:Point( "LEFT", self, "RIGHT", 5, -2 )
		debuffs.size = 34
		debuffs.num = 3
		debuffs.spacing = 3
		debuffs.initialAnchor = "LEFT"
	...
	if( unit and unit:find( "boss%d" ) and C["unitframes"].showboss == true ) then
		...
		buffs:SetHeight( 34 )
		buffs:SetWidth( 200 )
		buffs:Point( "TOPRIGHT", self, "TOPLEFT", -4, 2 )
		buffs.size = 34
		buffs.num = 3
		buffs.spacing = 3
