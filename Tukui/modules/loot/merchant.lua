...
f:SetScript( "OnEvent", function()
	if( C["merchant"].sellgrays == true or C["merchant"].sellmisc == true ) then
		local c = 0
		for b = 0, 4 do
			for s = 1, GetContainerNumSlots( b ) do
				...
				if( l and lid ) then
					...
					if( C["merchant"].sellgrays == true and select( 3, GetItemInfo( l ) ) == 0 and p > 0 ) then
						UseContainerItem( b, s )
						PickupMerchantItem()
						c = c + p
					end
					if( C["merchant"].sellmisc == true and filter[ lid ] ) then
						UseContainerItem( b, s )
						PickupMerchantItem()
						c = c + p
					end
	...
	if not IsShiftKeyDown() then
		if( CanMerchantRepair() and C["merchant"].autorepair == true ) then
			local cost, possible = GetRepairAllCost()
			if( C["merchant"].guildrepair == true ) then
			...
			if( cost > 0 ) then
				if( possible ) then
					...
					if( guildRepairFlag == 1 ) then
						RepairAllItems( 1 )
						DEFAULT_CHAT_FRAME:AddMessage( L.merchant_repair_guild .. " |cffffffff" .. g .. L.goldabbrev .. " |cffffffff" .. s .. L.silverabbrev .. " |cffffffff" .. c .. L.copperabbrev .. ".", 255, 255, 0 )
					else
						RepairAllItems()
						DEFAULT_CHAT_FRAME:AddMessage( L.merchant_repair_personal .. " |cffffffff" .. g .. L.goldabbrev .. " |cffffffff" .. s .. L.silverabbrev .. " |cffffffff" .. c .. L.copperabbrev .. ".", 255, 255, 0 )
					end
				else
