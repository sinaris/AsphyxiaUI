...
function S.StyleActionBarButton( self )
	...
	if not _G[name.."Panel"] then
		if( self:GetHeight() ~= S.buttonsize and not InCombatLockdown() ) then
			self:SetSize( S.buttonsize, S.buttonsize )
		end

		local panel = CreateFrame( "Frame", name .. "Panel", self )
		panel:CreatePanel( "Default", S.buttonsize, S.buttonsize, "CENTER", self, "CENTER", 0, 0 )
...
function S.StyleActionBarPetAndShiftButton( normal, button, icon, name, pet )
	button:SetNormalTexture( "" )
	...
	if not _G[name .. "Panel"] then
		button:SetWidth( S.petbuttonsize )
		button:SetHeight( S.petbuttonsize )

		local panel = CreateFrame( "Frame", name .. "Panel", button )
		panel:CreatePanel( "Default", S.petbuttonsize, S.petbuttonsize, "CENTER", button, "CENTER", 0, 0 )
		...
		icon:ClearAllPoints()
		if( pet ) then
			if( S.petbuttonsize < 30 ) then
				local autocast = _G[name .. "AutoCastable"]
				autocast:SetAlpha( 0 )
			end
			local shine = _G[name .. "Shine"]
			shine:Size( S.petbuttonsize, S.petbuttonsize )
...
function S.StyleShift()
	for i = 1, NUM_SHAPESHIFT_SLOTS do
		local name = "ShapeshiftButton" .. i
		local button  = _G[name]
		local icon  = _G[name .. "Icon"]
		local normal  = _G[name .. "NormalTexture"]
		S.StyleActionBarPetAndShiftButton( normal, button, icon, name )
	end
end
...
function S.StylePet()
	for i = 1, NUM_PET_ACTION_SLOTS do
		...
		S.StyleActionBarPetAndShiftButton( normal, button, icon, name, true )
	end
end

function S.UpdateActionBarHotKey( self, actionButtonType )
...
local function SetupFlyoutButton()
	for i = 1, buttons do
		if( _G["SpellFlyoutButton" .. i] ) then
			S.StyleActionBarButton( _G["SpellFlyoutButton" .. i] )
...
function S.StyleActionBarFlyout( self )
...
hooksecurefunc( "ActionButton_Update", S.StyleActionBarButton )
hooksecurefunc( "ActionButton_UpdateHotkeys", S.UpdateActionBarHotKey )
hooksecurefunc( "ActionButton_UpdateFlyout", S.StyleActionBarFlyout )
